...
import {
  FolderOpen, ChevronRight, ArrowLeft, Inbox, Search,
  FileText, Ruler, ClipboardList, Settings, Truck,
  Calculator, FileEdit, Handshake, CheckCircle, Users,
  ArrowRightCircle, BarChart3, TrendingUp,
} from "lucide-react";
...
const iconMap: Record<string, React.ElementType> = {
  ...
  "check-circle": CheckCircle, users: Users,
  "arrow-right-circle": ArrowRightCircle, "bar-chart-3": BarChart3,
...
export default function StepFolderGrid({ company, project, files, onSelectStep, onBack }: StepFolderGridProps) {
  ...
  const getFolderFileCount = (folderName: string): number => {
    const stepIds = flowNodes.filter((n) => n.folderName === folderName).map((n) => n.id);
    return files.filter((f) => stepIds.includes(f.stepId)).length;
  };

  const getStepFileCount = (stepId: string): number => {
    return files.filter((f) => f.stepId === stepId).length;
  };
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{files.length} fichiers sur {uniqueFolders.length} dossiers</p>
          </div>
        </div>
      </div>
          ...
          return (
            <div key={folderName} className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{folderCount} fichier{folderCount > 1 ? "s" : ""}</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pl-6">
                {steps.map((step) => {
                  const Icon = step.icon ? iconMap[step.icon] : FileText;
                  const count = getStepFileCount(step.id);
                  return (
                    <button
                      key={step.id}
                    ...
                    >
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${step.type === "start" ? "bg-[#1E3A5F] text-white" : step.type === "step" ? "bg-gray-100 text-gray-600 group-hover:bg-[#1E3A5F]/10 group-hover:text-[#1E3A5F]" : step.type === "decision" ? "bg-red-50 text-red-500" : step.type === "parallel" ? "bg-blue-50 text-blue-600" : "bg-green-50 text-green-600"}`}>
                        <Icon size={18} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-800 truncate">{step.step}. {step.title}</p>
                        <p className="text-xs text-gray-400 mt-0.5">{count} fichier{count > 1 ? "s" : ""}</p>
                      </div>
